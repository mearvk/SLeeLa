...
typedef struct {
    uint64_t stream_id; uint64_t request_id; uint64_t segment_id;
    uint32_t total_length; uint32_t segment_count; uint32_t received_segments;
    size_t received_bytes; uint8_t *buffer; uint8_t *bitmap; uint8_t *coverage;
} http4_reassembly_t;
...
int http4_reassembly_add(http4_reassembly_t *state, const http4_segment_view_t *segment);
