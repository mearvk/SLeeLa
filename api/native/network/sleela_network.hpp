...
public:
 ...
 Status open(Transport transport, AddressFamily family=AddressFamily::Any);
 Status bind(const Endpoint&);
 Status connect(const Endpoint&);
 Status send(const std::vector<std::uint8_t>&);
 ReceiveResult receive(std::size_t maxBytes=65536);
 Status close();
...
class TcpConnection {
public: TcpConnection(); ~TcpConnection(); Status connect(const Endpoint&); Status send(const std::vector<std::uint8_t>&); ReceiveResult receive(std::size_t maxBytes=65536); Status close();
