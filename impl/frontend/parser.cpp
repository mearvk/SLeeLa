...
ClassDecl Parser::parseClass(){expect(Tok::KwClass,"'class'");ClassDecl c;c.name=expect(Tok::Ident,"class name").text;expect(Tok::LBrace,"'{'");while(!check(Tok::RBrace)&&!check(Tok::Eof)){int save=(int)i_;accept(Tok::KwStatic);if(!isTypeStart())error("expected a type for a field or method");i_++;if(!check(Tok::Ident))error("expected a field or method name");Tok after=peek(1).kind;i_=(size_t)save;if(after==Tok::LParen)c.methods.push_back(parseMethod());else c.fields.push_back(parseField());}expect(Tok::RBrace,"'}'");return c;}
Field Parser::parseField(){accept(Tok::KwStatic);Field f;f.type=parseType();f.name=expect(Tok::Ident,"field name").text;if(accept(Tok::Assign))f.init=parseExpr();expect(Tok::Semicolon,"';'");return f;}
