...
if [[ -z "$CONFIG" || ! -f "$CONFIG" ]]; then
  echo "ERROR: set AE6E66_MAIL_CONFIG to an administrator-controlled mail configuration file." >&2
...
# Require a non-wildcard listener and a closed relay policy.
...
# Verify that DKIM is configured through a local Unix socket, not an exposed TCP listener.
milters="$(postconf -h smtpd_milters 2>/dev/null || true)"
if [[ "$milters" != *unix:* ]]; then
  echo "ERROR: expected a local Unix-socket DKIM milter." >&2
  exit 1
fi
