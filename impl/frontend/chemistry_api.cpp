...
ExprP C1(const std::string& name, ExprP a) { auto c=std::make_unique<Call>(name); c->args.push_back(std::move(a)); return c; }
ExprP C2(const std::string& name, ExprP a, ExprP b) { auto c=std::make_unique<Call>(name); c->args.push_back(std::move(a)); c->args.push_back(std::move(b)); return c; }
...
ExprP evidenceSum() {
    return B("+",
        B("+",B("+",B("*",D(.24),Clamp(V("observation"))),B("*",D(.20),Clamp(V("bond")))),B("*",D(.22),Clamp(V("valence")))),
        B("+",B("*",D(.13),Clamp(V("similarity"))),B("+",B("*",D(.12),Clamp(V("signal"))),B("*",D(.09),Clamp(V("symmetry"))))));
}
...
void addChemistry(Program& p) {
    ...
    m.body->stmts.push_back(Ret(B("+",D(0.98),B("*",B("*",D(0.04),C1("__native_math_abs",C2("__native_math_fmod",B("*",V("seed"),D(0.6180339887498949)),D(1)))),Clamp(V("weight"))))));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_inference_level");
    m.body->stmts.push_back(Ret(C2("__native_math_max",C2("__native_math_min",B("*",D(24),evidenceSum()),D(24)),D(0))));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_confidence");
    m.body->stmts.push_back(Ret(Clamp(evidenceSum())));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_uncertainty");
    m.body->stmts.push_back(Ret(B("-",D(1),Clamp(evidenceSum()))));
    c.methods.push_back(std::move(m));
