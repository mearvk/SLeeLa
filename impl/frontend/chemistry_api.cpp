...
ExprP C6(const std::string& name, ExprP a, ExprP b, ExprP c, ExprP d, ExprP e, ExprP f) {
    auto x=C(name); x->args.push_back(std::move(a)); x->args.push_back(std::move(b)); x->args.push_back(std::move(c));
    x->args.push_back(std::move(d)); x->args.push_back(std::move(e)); x->args.push_back(std::move(f)); return x;
}
...
void addChemistry(Program& p) {
    ...
    // Deterministic bounded perturbation. It is intentionally small enough to
    // rank candidates without manufacturing a new scientific observation.
    m.body->stmts.push_back(Ret(B("+",D(0.98),B("*",D(0.04),C2("__native_math_abs",C2("__native_math_fmod",B("*",V("seed"),D(0.6180339887498949)),D(1))))))));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_inference_level");
    auto weighted=B("+",B("+",B("+",B("*",D(.24),C1("__native_math_clamp",V("observation"))),B("*",D(.20),C1("__native_math_clamp",V("bond")))),B("*",D(.22),C1("__native_math_clamp",V("valence")))),B("+",B("*",D(.13),C1("__native_math_clamp",V("similarity"))),B("+",B("*",D(.12),C1("__native_math_clamp",V("signal"))),B("*",D(.09),C1("__native_math_clamp",V("symmetry"))))));
    m.body->stmts.push_back(Ret(C1("__native_math_clamp",B("*",D(24),weighted),D(0))));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_confidence");
    weighted=B("+",B("+",B("+",B("*",D(.24),C1("__native_math_clamp",V("observation"))),B("*",D(.20),C1("__native_math_clamp",V("bond")))),B("*",D(.22),C1("__native_math_clamp",V("valence")))),B("+",B("*",D(.13),C1("__native_math_clamp",V("similarity"))),B("+",B("*",D(.12),C1("__native_math_clamp",V("signal"))),B("*",D(.09),C1("__native_math_clamp",V("symmetry"))))));
    m.body->stmts.push_back(Ret(C1("__native_math_clamp",weighted,D(0))));
    c.methods.push_back(std::move(m));

    m=M6("double","__native_chemistry_uncertainty");
    weighted=B("+",B("+",B("+",B("*",D(.24),C1("__native_math_clamp",V("observation"))),B("*",D(.20),C1("__native_math_clamp",V("bond")))),B("*",D(.22),C1("__native_math_clamp",V("valence")))),B("+",B("*",D(.13),C1("__native_math_clamp",V("similarity"))),B("+",B("*",D(.12),C1("__native_math_clamp",V("signal"))),B("*",D(.09),C1("__native_math_clamp",V("symmetry"))))));
    m.body->stmts.push_back(Ret(B("-",D(1),C1("__native_math_clamp",weighted,D(0)))));
    c.methods.push_back(std::move(m));
