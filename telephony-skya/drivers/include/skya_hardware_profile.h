...
typedef enum { SKYA_VALUE_ACTUAL=0, SKYA_VALUE_DEFAULT_ASSUMPTION=1 } skya_value_status;
typedef struct { const char *name,*number; skya_pin_role role; skya_pin_direction direction; skya_pin_pull pull; int active_high; uint32_t voltage_min_mv,voltage_nominal_mv,voltage_max_mv,max_current_ma,flags; skya_evidence_level evidence; skya_value_status value_status; const char *assumption_note; } skya_pin_spec;
typedef struct { const char *name; skya_memory_type type; uint64_t address,size; uint32_t alignment; skya_cache_policy cache; int readable,writable,executable,volatile_storage; skya_evidence_level evidence; skya_value_status value_status; const char *assumption_note; } skya_memory_region;
typedef struct { uint32_t cache_line_bytes,read_ahead_bytes,write_buffer_bytes; skya_cache_policy default_policy; int dma_coherent,requires_flush,requires_invalidate; skya_value_status value_status; const char *assumption_note; } skya_cache_spec;
typedef struct { const char *vendor,*model,*family,*hardware_revision,*firmware_version,*interface_revision; skya_evidence_level identity_evidence; const skya_pin_spec *pins; size_t pin_count; const skya_memory_region *memory; size_t memory_count; skya_cache_spec cache; uint32_t flags; skya_value_status value_status; const char *assumption_note; } skya_hardware_profile;
