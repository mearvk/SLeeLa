...
```sh
cd impl && make                                  # builds build/sleela and build/nordshrift
export SLEELA_SHEET=$PWD/../SHEET.sheet          # binds the SHEET.sheet catalog

# run from the repo root against this top-level examples/ copy:
./impl/build/nordshrift check examples/symmetry/build.sst   # validate the sheet
./impl/build/nordshrift build examples/symmetry/build.sst   # transpile + run all sources
```

> A mirror of this module also lives under `impl/nordshrift/examples/symmetry/`
> (wired into the `make test` targets). Both copies are identical.
...
- Run a single file through **`sleela run`** with `SLEELA_SHEET` set, the catalog
  ...
  ```sh
  ./impl/build/sleela run examples/symmetry/src/core/Symmetry.sleela
  # system depth  = 3024
  # degree max    = 4
  # Service role  = root
