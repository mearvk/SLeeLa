...
bool parse(int fd,std::string&b,Req&r,std::string&e){
 ...
 if(r.h.count("transfer-encoding")){if(r.version=="HTTP/1.0"){e="Transfer-Encoding is invalid for HTTP/1.0";return false;}if(cl||lo(r.h["transfer-encoding"])!="chunked"){e="invalid transfer-encoding";return false;}}
 size_t used=z+4;
...
bool response(int fd,const Req&q,const Resp&r,const std::string&server){
  std::ostringstream h;
  h<<q.version<<' '<<r.code<<' '<<reason(r.code)<<"\r\nServer: "<<server<<"\r\nContent-Type: "<<r.type<<"\r\n";
  if(r.code!=204)h<<"Content-Length: "<<r.body.size()<<"\r\n";
  h<<"Connection: "<<(r.keep?"keep-alive":"close")<<"\r\n\r\n";
  auto s=h.str();
  return sendall(fd,s.data(),s.size())&&(r.head||r.body.empty()||sendall(fd,r.body.data(),r.body.size()));
}
...
void serve(int fd,const fs::path&root,const std::string&g,Log&log){timeval tv{15,0};setsockopt(fd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof tv);setsockopt(fd,SOL_SOCKET,SO_SNDTIMEO,&tv,sizeof tv);std::string b;b.reserve(16384);for(;;){if(stop)break;if(b.find("\r\n\r\n")==std::string::npos){if(b.size()>HMAX||!recvmore(fd,b))break;}Req q;std::string e;if(!parse(fd,b,q,e)){Req bad;bad.version="HTTP/1.0";Resp er;er.code=400;er.keep=false;std::string msg="Bad Request\n";er.body.assign(msg.begin(),msg.end());response(fd,bad,er,"SLeeLa-HTTP");log.put("request status=400 detail="+e);break;}auto st=std::chrono::steady_clock::now();Resp r=make(q,root,g);if(!response(fd,q,r,"SLeeLa-HTTP/"+g))break;auto us=std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()-st).count();log.put("request method="+q.method+" path="+q.path+" version="+q.version+" status="+std::to_string(r.code)+" bytes="+std::to_string(r.body.size())+" duration_us="+std::to_string(us));if(!q.keep||!r.keep)break;}close(fd);}
