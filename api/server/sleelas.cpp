...
#endif
    std::error_code ec;
    return fs::absolute(fs::path(argv0 ? argv0 : "sleelas"), ec).parent_path();
}

static bool regular_file(const fs::path &p) {
    std::error_code ec;
...
static fs::path locate_root(const fs::path &exe_dir) {
    if (const char *env = std::getenv("SLEELA_ROOT"); env && *env) {
        fs::path p(env);
        if (regular_file(p / "server-edition/src/Server.sleela")) return p;
    }
    std::vector<fs::path> candidates = {
        exe_dir, exe_dir.parent_path(), exe_dir.parent_path().parent_path(), fs::current_path()
    };
...
#endif
    if (const char *env = std::getenv("SLEELA_BIN"); env && *env && regular_file(env))
        return fs::absolute(env);
    for (const auto &p : {root / "bin" / name, root / "impl" / "build" / name})
        if (regular_file(p)) return p;
    return {};
}
...
static void help() {
    std::cout
      ...
      << "  --foreground    echo the server result log tail\n"
      << "  --help          show this help\n\n"
      << "Environment:\n"
      << "  SLEELA_ROOT               repository/install root\n"
      << "  SLEELA_BIN                SLeeLa engine path\n"
      << "  SLEELA_SHA256_MANIFEST    trusted execution manifest\n";
}
...
#if defined(_WIN32)
static int run_engine(const fs::path &root, const fs::path &engine,
    ...
    if (!CreateProcessA(nullptr, mutable_cmd.data(), nullptr, nullptr, FALSE,
                        0, nullptr, root.string().c_str(), &si, &pi)) {
...
static int run_engine(const fs::path &root, const fs::path &engine,
    ...
    if (pid == 0) {
        ...
        std::vector<std::string> args = {engine.string(), "run", server.string()};
        std::vector<char*> av;
        for (auto &s : args) av.push_back(s.data());
        av.push_back(nullptr);
        execv(engine.c_str(), av.data());
        std::perror("sleelas: exec");
        std::exit(127);
    }
    int status = 0;
...
int main(int argc, char **argv) {
    ...
    if (root.empty()) {
        std::cerr << "sleelas: SLeeLa root not found; set SLEELA_ROOT\n";
        return 1;
    }
    ...
    if (engine.empty()) {
        std::cerr << "sleelas: SLeeLa engine not found; set SLEELA_BIN or build impl/\n";
        return 1;
    }
    ...
    std::error_code ec;
    fs::create_directories(inbox.parent_path(), ec);
    fs::create_directories(state, ec);
    if (ec) { std::cerr << "sleelas: cannot create server state: " << ec.message() << "\n"; return 1; }

    if (!fs::create_directory(lock, ec)) {
        std::cerr << "sleelas: another server run is active\n";
        return 3;
    }
    struct Unlock { fs::path p; ~Unlock() { std::error_code e; fs::remove(p, e); } } unlock{lock};

    if (!regular_file(inbox)) {
        std::ofstream out(inbox);
        out << "hello\n";
    }
    if (tick) {
...
#if defined(_WIN32)
            _putenv_s("SLEELA_SHA256_MANIFEST", manifest.string().c_str());
#else
            setenv("SLEELA_SHA256_MANIFEST", manifest.c_str(), 0);
#endif
        }
    }

    if (foreground)
        std::cout << "sleelas: starting Server.sleela using " << engine << "\n";

    const int rc = run_engine(root, engine, server);
    if (rc != 0) {
        std::cerr << "sleelas: server reference exited with status " << rc << "\n";
        return rc;
    }

    std::cout << "sleelas: processed server-edition/src/Server.sleela -> "
              << log << "\n";

    if (foreground && regular_file(log)) {
        std::ifstream in(log);
        std::string line;
        ...
        for (const auto &s : tail) std::cout << s << '\n';
    }
    return 0;
}
