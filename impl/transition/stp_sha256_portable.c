// ===========================================================================
// stp_sha256_portable.c -- a compact, dependency-free SHA-256 (FIPS 180-4).
// Used to compute the STP parse digest in any build (crypto or not).
// ===========================================================================
#include "stp_crypto_optional.h"
#include <string.h>
...
void stp_sha256_portable(const uint8_t* data, size_t len, uint8_t out32[32]) {
    ...
    for (size_t i = 0; i < padded; i += 64) sha256_block(h, tail + i);

    for (int i = 0; i < 8; i++) {
        out32[i*4]   = (uint8_t)(h[i] >> 24);
