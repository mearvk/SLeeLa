...
static const char* kVersion="Sleelvac™ 1.4 (Sleela compiler; executable native math/physics/economics/chemistry/financial modules; persistent .sleela Core artifacts; .xclass input; OS Defender provisioning)";
...
static int defenderCmd(int argc,char**argv){
    ...
    if(action=="detect"){std::cout<<"OS defender: "<<defenderName()<<"\nRepository: "<<defenderRepo()<<"\n";return 0;}
    if(action=="fetch")return defenderFetch(base);
    if(action=="build"){if(!fs::exists(source)&&defenderFetch(base))return 1;return defenderBuild(source);}
    if(action=="install"){if(!fs::exists(source)&&defenderFetch(base))return 1;if(defenderBuild(source))return 1;return defenderInstall(source);}
    if(action=="provision"){if(defenderFetch(base))return 1;if(defenderBuild(source))return 1;return defenderInstall(source);}
...
static int checkFile(const std::string&path){std::string src;sleela::Program prog;sleela::VersionResolution v;if(!parseSource(path,src,prog,v))return 1;try{sleela::chemistry::lowerProgram(prog);sleela::financial::lowerProgram(prog);}catch(const std::exception&ex){std::cerr<<"sleelvac: "<<path<<": "<<ex.what()<<"\n";return 1;}std::cout<<path<<": ok (syntax "<<(v.pragmaPresent?"declared ":"assumed ")<<v.declared.str()<<")\n";return 0;}
...
int main(int argc,char**argv){if(argc<2)return usage();std::string cmd=argv[1];if(cmd=="version"||cmd=="--version"||cmd=="-v"){std::cout<<kVersion<<"\n";std::cout<<"  supported .sleela syntax: "<<sleela::minSupportedSyntax().str()<<" .. "<<sleela::maxSupportedSyntax().str()<<"\n";return 0;}if(cmd=="compile"){if(argc!=5||std::string(argv[3])!="-o")return usage();return compileFile(argv[2],argv[4]);}if(cmd=="check"){if(argc<3)return usage();return checkFile(argv[2]);}if(cmd=="run"){if(argc<3)return usage();if(hasExt(argv[2],".xclass")){std::vector<std::string>files;for(int i=2;i<argc;i++)files.push_back(argv[i]);return runXclass(files);}return runFile(argv[2]);}if(cmd=="xclass"){if(argc<3)return usage();return xclassCmd(argc,argv);}if(cmd=="defender")return defenderCmd(argc,argv);if(hasExt(cmd,".sleela")||hasExt(cmd,".xclass"))return runFile(cmd);return usage();}
