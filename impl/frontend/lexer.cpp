...
const char* tokName(Tok t) {
    switch (t) {
        ...
        case Tok::KwDoubleT: return "double"; case Tok::KwBoolT: return "boolean";
        case Tok::KwStringT: return "String"; case Tok::KwIf: return "if";
        case Tok::KwElse: return "else"; case Tok::KwWhile: return "while";
        case Tok::KwFor: return "for"; case Tok::KwReturn: return "return";
        ...
        case Tok::Dot: return "."; case Tok::Assign: return "=";
        case Tok::Plus: return "+"; case Tok::Minus: return "-";
        case Tok::Star: return "*"; case Tok::Slash: return "/";
        case Tok::Percent: return "%"; case Tok::EqEq: return "==";
        case Tok::NotEq: return "!="; case Tok::Lt: return "<";
        case Tok::Le: return "<="; case Tok::Gt: return ">";
        case Tok::Ge: return ">="; case Tok::AndAnd: return "&&";
        case Tok::OrOr: return "||"; case Tok::Not: return "!";
...
Token Lexer::makeNumber() {
    int startLine = line_, startCol = col_; std::string s; bool isDouble = false;
    while (std::isdigit((unsigned char)peek())) s += advance();
    if (peek() == '.' && std::isdigit((unsigned char)peek(1))) {
        isDouble = true; s += advance(); while (std::isdigit((unsigned char)peek())) s += advance();
    }
    return Token{isDouble ? Tok::Double : Tok::Int, s, startLine, startCol};
}
Token Lexer::makeString() {
    int startLine = line_, startCol = col_; advance(); std::string out;
    while (!atEnd() && peek() != '"') {
        char c = advance();
        if (c == '\\') {
            char e = advance();
...
Token Lexer::makeIdentOrKeyword() {
    ...
    auto it = kw.find(s); Tok t = (it != kw.end()) ? it->second : Tok::Ident;
...
std::vector<Token> Lexer::tokenize() {
    ...
    for (;;) {
        ...
        int L=line_, C=col_; char c=peek();
        ...
        switch(c) {
            ...
            case '.': emit(Tok::Dot,"."); break; case '+': emit(Tok::Plus,"+"); break;
            case '-': emit(Tok::Minus,"-"); break; case '*': emit(Tok::Star,"*"); break;
            case '/': emit(Tok::Slash,"/"); break; case '%': emit(Tok::Percent,"%"); break;
