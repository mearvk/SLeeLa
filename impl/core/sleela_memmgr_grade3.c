...
int slmm_g3_begin(SLMMGrade3*m,SLMMG3Resource r,size_t b,uint64_t h){if(!m||!m->initialized||!okr(r))return -1;if(b>SIZE_MAX-m->live_memory||m->live_memory+b>m->memory_limit){m->failure_count++;m->resource[r].failures++;return -1;}m->live_memory+=b;if(m->live_memory>m->peak_memory)m->peak_memory=m->live_memory;SLMMG3Stats*s=&m->resource[r];s->calls++;s->bytes+=b;if(h>UINT64_MAX-s->live_handles){m->failure_count++;s->failures++;return -1;}s->live_handles+=h;return 0;}
...
void slmm_g3_fail(SLMMGrade3*m,SLMMG3Resource r){if(!m||!m->initialized||!okr(r))return;m->failure_count++;m->resource[r].failures++;}
