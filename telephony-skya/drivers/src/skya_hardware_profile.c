...
int skya_hardware_pin_set(skya_hardware_control*c,const skya_pin_spec*p,int v){if(!c||!c->set_pin||!p)return-EINVAL;if(p->direction==SKYA_PIN_INPUT_ONLY)return-EPERM;return c->set_pin(c->ctx,p,v?1:0);}
...
int skya_hardware_voltage_set(skya_hardware_control*c,const skya_pin_spec*p,uint32_t mv){if(!c||!c->set_voltage||!p)return-EINVAL;if(p->role!=SKYA_PIN_POWER_RAIL||!p->voltage_max_mv||mv<p->voltage_min_mv||mv>p->voltage_max_mv)return-ERANGE;return c->set_voltage(c->ctx,p,mv);}
...
int skya_hardware_memory_write(skya_hardware_control*c,const skya_memory_region*r,uint64_t o,const void*b,size_t n){if(!c||!c->write_memory||!r||(!b&&n)||!r->writable)return-EINVAL;int e=bounds(r,o,n);return e?e:c->write_memory(c->ctx,r,o,b,n);}
