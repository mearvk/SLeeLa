...
std::vector<Instruction> Decoder::decode(const Artifact& a, std::uint64_t address, std::size_t length) const {
    ...
    if (start >= b.size()) return out;
    const std::size_t end = std::min(b.size(), start + length);
    for (std::size_t p = start; p < end; ++p) {
        Instruction i;
        i.address = p;
        i.bytes.push_back(b[p]);
        i.mnemonic = "db";
        i.operands.push_back({Operand::Kind::Immediate, "0x" + std::to_string(b[p]), b[p]});
        i.provenance = {p, p, "raw-byte", 1.0};
