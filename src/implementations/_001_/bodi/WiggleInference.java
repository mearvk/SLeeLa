 ...
 *
 * The inference pass produces structural hypotheses only. It does not invoke
 * Bodi verbs, perform remote changes, or treat correlation as causation.
 */
public final class WiggleInference
{
    ...
    public static WiggleStructuralProfile infer(String xml) throws Exception
    {
        ...
        try { factory.setFeature(XMLConstants.FEATURE_SECURE_PROCESSING, true); }
    ...
    public static WiggleStructuralProfile infer(Document document)
    {
        ...
        // A profile represents one bounded inference envelope. Five is the
        // normal Wiggle search budget even though the structural walk is local.
    ...
    private static void walk(Element element, int depth, Metrics metrics)
    {
        ...
        for (int i = 0; i < attributes.getLength(); i++)
        {
            ...
            metrics.attributeNames.add(name);
            if (looksLikeReference(name, value))
                metrics.referenceCandidates++;
            if (looksLikeState(name, value))
                metrics.stateSignals++;
            if (looksLikeCommand(name, value))
                metrics.commandSignals++;
            if (looksLikeDescription(name, value))
                metrics.descriptionSignals++;
        }
        ...
        {
            if (count.intValue() > 1)
                metrics.repeatedKinds++;
        }
        if (elementChildren > 1)
            metrics.orderedParents++;
        if (elementChildren == 2)
            metrics.relationSignals++;
        if (elementChildren > 3)
            metrics.recordSignals++;
    }
    ...
    private static boolean looksLikeReference(String name, String value)
    {
        return name.endsWith("id") || name.endsWith("ref") || name.endsWith("uri")
            ...
            || (value != null && (value.startsWith("urn:") || value.startsWith("http://")
