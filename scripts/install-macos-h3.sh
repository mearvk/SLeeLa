#!/bin/bash
# =============================================================================
# install-macos-h3.sh — Install the SLeeLa HTTP 3.0 module (app) on macOS
# =============================================================================
#
# macOS (Darwin) counterpart of install-linux-h3.sh. Installs the packet-builder
# + HTTP/2 listener stack that lets the SLeeLa HTTP 3.0 envelope travel the
# existing internet:
...
#
# Usage:
#   ./scripts/install-macos-h3.sh                 # build + stage into PREFIX
#   PREFIX=/opt/sleela ./scripts/install-macos-h3.sh
#   CATALINA_HOME=/opt/homebrew/opt/tomcat/libexec ./scripts/install-macos-h3.sh
#
# Environment variables (all optional):
#   PREFIX          Install root                 (default: /usr/local/sleela/h3)
...
#
# Requirements:
#   - bash, coreutils (macOS ships BSD install(1), which supports the flags used)
#   - python3 (3.10+) for the packet builder (e.g. `brew install python`)
#   - a JDK (javac) + a Jakarta Servlet 6.0 API jar to build the HTTP module
#   - (optional) Apache Tomcat 10.1+ (`brew install tomcat`) and Apache httpd
#     with mod_http2 (`brew install httpd`)
# =============================================================================
set -euo pipefail
...
# --- Configuration -----------------------------------------------------------
PREFIX="${PREFIX:-/usr/local/sleela/h3}"
...
[ "$(uname -s)" = "Darwin" ] || warn "not running on macOS (uname=$(uname -s)); continuing anyway"
...
log "Apache HTTP/2 front (Homebrew httpd): enable http2/proxy/proxy_http2 modules"
log "in httpd.conf, then include deploy/apache-h3.conf and reload: apachectl -k graceful"
