...
sed -i "s#^recovery_log=.*#recovery_log=$PREFIX/server-edition/state/recovery.log#" "$DEST/config/server.conf"
...
sed "s#/opt/sleela#$PREFIX#g" "$DEST/service/sleela-server3.service" > /etc/systemd/system/sleela-server3.service
command -v systemctl >/dev/null 2>&1 && systemctl daemon-reload || true
printf '%s\n' "Installed SLeeLa Server Edition Service 3 at $DEST"
